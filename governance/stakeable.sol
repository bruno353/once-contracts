...
contract ERC20Stakeable is ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    //removing addresses from array:

    function remove(uint _index) internal {
        require(_index < stakersArr.length, "index out of bound");

        for (uint i = _index; i < stakersArr.length - 1; i++) {
            stakersArr[i] = stakersArr[i + 1];
        }
        stakersArr.pop();
    }
}
